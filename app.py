...
def download_order_csv():
    """Download the processed order CSV file."""
    try:
        # Get existing processor with session directory
        processor = get_or_create_session()
        ...
        if not os.path.exists(csv_path):
            return jsonify({'error': 'No processed order file found. Please upload and process an order PDF first.'}), 404
            
        return send_file(csv_path, as_attachment=True, download_name="order_data.csv")
    except Exception as e:
        return jsonify({'error': str(e)}), 500
